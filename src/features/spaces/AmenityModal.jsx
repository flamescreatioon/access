...
import { X, Sparkles, Type, Info } from 'lucide-react';

export default function AmenityModal({ open, data, onClose, onSave }) {
    ...
    useEffect(() => {
        ...
        else setFormData({ name: '', description: '', icon: 'Sparkles' });
    }, [data, open]);
            ...
            <div className="relative w-full max-w-md bg-white dark:bg-surface-800 rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                <div className="p-6 border-b border-surface-100 dark:border-surface-700 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-bold">{data ? 'Edit Amenity' : 'New Amenity'}</h2>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    <div>
                        <label className="block text-xs font-bold text-surface-500 uppercase tracking-widest mb-1.5 ml-1">Amenity Name</label>
                    ...
                    <div>
                        <label className="block text-xs font-bold text-surface-500 uppercase tracking-widest mb-1.5 ml-1">Description (Optional)</label>
                        <div className="relative">
                            <Info className="absolute left-4 top-4 w-4 h-4 text-surface-400" />
                            <textarea
                                value={formData.description}
                                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                                className="w-full pl-11 pr-4 py-3 rounded-xl bg-surface-50 dark:bg-surface-900 border-none focus:ring-2 focus:ring-primary-500 transition-all font-medium min-h-[80px]"
                    ...
                    <div className="flex gap-3 pt-4">
                        ...
                        >
                            {data ? 'Update Amenity' : 'Create Amenity'}
                        </button>
                    </div>
                </form>
            </div>
