...
export default function AdminMemberList() {
    const { members, fetchAllMembers, suspendMember, reactivateMember, tiers, updateMemberTier, isLoading } = useMembershipStore();
    ...
    const filtered = members.filter(m => {
        ...
        const matchSearch = !search || `${name} ${email}`.toLowerCase().includes(search.toLowerCase());
                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {filtered.map(m => {
                                ...
                                const tierName = m.AccessTier?.name || 'N/A';
                                const tierColor = tierName === 'VIP' ? '#eab308' : tierName === 'Pro' ? '#22c55e' : tierName === 'Enterprise' ? '#f97316' : '#64748b';
                                    ...
                                    <tr key={m.id} className="border-b border-surface-100 dark:border-surface-700/50 hover:bg-surface-50 dark:hover:bg-surface-800/50 transition-colors">
                                        <td className="px-5 py-3">
                                            <div className="flex items-center gap-3">
                                                <div className="w-9 h-9 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center text-primary-600 dark:text-primary-400 text-xs font-bold">
                                                    {initials}
                                                </div>
                                                <div>
                                        ...
                                        <td className="px-5 py-3">
                                            <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${statusStyles[status] || 'bg-surface-100 text-surface-500'}`}>
                                                {status}
                                            </span>
                                        </td>
                                        <td className="px-5 py-3 hidden lg:table-cell text-sm text-surface-500">
                                            {m.expiry_date ? format(new Date(m.expiry_date), 'MMM d, yyyy') : 'N/A'}
                                        </td>
